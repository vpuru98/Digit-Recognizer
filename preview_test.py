...
"""
Preview first hundred images of the test set
Use forward and backward arrow keys to preview next and previous images

"""

import pygame
from utility import get_test_image_vector, get_test_image_label

WIDTH = 700
HEIGHT = 700

window = pygame.display.set_mode((WIDTH, HEIGHT))
pygame.display.set_caption("Preview Test Set : ")
...
def retrieve_image(image_number):
    ...
    display_image_vector = get_test_image_vector(image_number)
    image_label = get_test_image_label(image_number)
    pygame.display.set_caption(
        "Preview Test Set : Displaying image [%i] with label %i" % (image_number, image_label))
    redraw_screen()
